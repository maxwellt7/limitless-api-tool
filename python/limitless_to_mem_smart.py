...
LAST_PROCESSED_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "last_processed_mem_smart.json")
...
def process_with_mem_it(conversation):
    """
    Process a single conversation with the Mem It API
    """
    title = conversation.get("title", "Untitled conversation")
    content = conversation.get("markdown", "")
    start_time = conversation.get("startTime", "")
    end_time = conversation.get("endTime", "")
    
    # Format input for Mem It
    input_text = f"# {title}\n\n"
    ...
    if start_time and end_time:
        ...
        try:
            ...
            input_text += f"Time: {time_str}\n\n"
        except ValueError:
            # If we can't parse the timestamps, use them as-is
            input_text += f"Start: {start_time}\nEnd: {end_time}\n\n"
    ...
    # Using Mem It API for intelligent processing
    data = {
        "input": input_text,
        "instructions": "Extract key points, action items, and important information from this conversation. Organize appropriately.",
        "context": "This is a transcribed conversation from Limitless AI",
        "timestamp": end_time if end_time else datetime.now(timezone.utc).isoformat()
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Check if there's a created note operation
            note_url = None
            for operation in response_data.get("operations", []):
                if operation.get("type") == "created-note":
                    note_url = operation.get("url")
                    note_title = operation.get("title")
                    break
            
            if note_url:
                print(f"Successfully processed: {title}")
                print(f"Mem.ai note: {note_title}")
                print(f"URL: {note_url}")
                return True
            else:
                print(f"No note created for: {title}")
                return False
        else:
            print(f"Error processing with Mem It API: {response.status_code}")
            print(response.text)
            return False
    
    except Exception as e:
        print(f"Exception processing with Mem It API: {e}")
        return False
...
def main():
    ...
    # Process each conversation individually with Mem It
    success_count = 0
    for lifelog in lifelogs:
        print(f"Processing: {lifelog.get('title', 'Untitled')}")
        if process_with_mem_it(lifelog):
            success_count += 1
            # Save the processed ID
            save_last_processed(
                lifelog.get("id", ""),
                lifelog.get("endTime", datetime.now(timezone.utc).isoformat())
            )
    
    print(f"Mem.ai sync complete! Successfully processed {success_count} of {len(lifelogs)} conversations.")
